...
void setup() {
 ...
 pinMode(47,INPUT);
  ...
  while(1)
  {
    if(millis()-timer>4000||digitalRead(47)==0) break;
    delay(20);
  }
 sportUPDN(0);
...
void spiderwalk(int i){
  for(int j=1;j<=i;j++){
    SCPUT(1760,1723,1043,1694,707,1103,1975,806,56,2179,319,718,1108,2203,1326,1617);//1
    scall();
    delay(600);
    SCPUT(1007,1691, 270,1750,140,1103,1412,806,573,2180,1098,618,1837,2237,1907,1617);//2
    scall();
    delay(600);
    speedsc=0.8;
    SCPUT(1003,1250, 267,2240,707,1103,1975,806,56,2179,1098,1300,1836,1650,1326,1617);//3
    scall();
    delay(1200);
    speedsc=1.4;
    SCPUT(1763,1220,1042,2259,140,1103,1412,806,573,2180,291,1300,1108,1650,1907,1617);//4
    scall();
    delay(600);
  }
}
...
void scall()
{
  ...
  for (int scalli=0;scalli<16;scalli++)
  {
    ...
    if(abs(LRold[scalli]-LRnow[scalli])>0){gospeed=1000*float(abs(LRold[scalli]-LRnow[scalli]))/(rtime);if(gospeed>1500)gospeed=1500;scmain.RegWritePosEx(rqd, LRnow[scalli]+deltaLR[scalli], gospeed,950);}
...
zhuandong=zhuandong*15;
if(zhuandong>255)zhuandong=160;
if(zhuandong<-255)zhuandong=-160;
if(zhuandong<40&&zhuandong>-40)zhuandong=0;
if(zhuandong>90||zhuandong<-90)speedx=0; else speedx=speed;
  Serial.println(znzget());
